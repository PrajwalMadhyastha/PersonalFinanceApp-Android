...
class MockCustomSmsRuleDao(private val rules: List<CustomSmsRule> = emptyList()) : CustomSmsRuleDao {
    override suspend fun insert(rule: CustomSmsRule) {
        // Not needed for these tests
    }

    override fun getRulesForSender(sender: String): Flow<List<CustomSmsRule>> {
        return flowOf(rules.filter { it.smsSender == sender })
    }
}

class SmsParserTest {
    private val emptyMappings = emptyMap<String, String>()
    private val emptyDao = MockCustomSmsRuleDao()
    ...
    fun `test parses debit message successfully`() = runBlocking {
        ...
        val result = SmsParser.parse(mockSms, emptyMappings, emptyDao)
    ...
    @Test
    fun `test parses HDFC NEFT credit message`() = runBlocking {
    ...
    @Test
    fun `test applies custom merchant rule correctly`() = runBlocking {
        val smsBody = "Transaction alert: A purchase of 12.50 has been made at a store called My-Unique-Store on your card."
        val mockSms = SmsMessage(id = 20L, sender = "TX-ALERT", body = smsBody, date = System.currentTimeMillis())
        val customRule = CustomSmsRule(
            smsSender = "TX-ALERT",
            ruleType = "MERCHANT",
            regexPattern = "at a store called (.*?) on",
            priority = 100
        )
        val mockDao = MockCustomSmsRuleDao(listOf(customRule))

        val result = SmsParser.parse(mockSms, emptyMappings, mockDao)

        assertNotNull(result)
        assertEquals("My-Unique-Store", result?.merchantName)
        // Amount should still be parsed by the fallback logic
